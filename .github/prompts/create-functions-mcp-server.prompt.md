...
- Create the Azure Functions project structure in the root of the MCP server project (e.g., folder where `package.json`, or `*.csproj` is located). If you can't find the root, ask for it. Specifically it is the MonkeyMCPSSE
...
Duplicate the current codebase to a new folder named `<original-folder-name>Remote` where `<original-folder-name>` is the name of the current folder. This is to ensure that the original MCP server code remains unchanged.
